...
> 📌 2026-08-07 追記: **この記事が挙げた5原則には抜けがあった。** 半年後にテーマを
> 監査したら、意味体系は筋が通ったままコントラストが崩れていた。末尾の「原則6」を参照。
...
## 原則 6: 意味を決めた後、**測る**（2026-08-07 追記）

上の5原則には抜けがあった。**どれ一つとしてコントラスト比に触れていない。**
その結果 v1.0.0 は、意味体系としては筋が通ったまま、**WCAG AA (4.5:1) に 15 箇所で不合格**だった。

| 要素 | v1.0.0 | 比 | 修正後 |
|---|---|---|---|
| Crimson（キーワード） | `#DC143C` | **3.97:1** ❌ | `#EC3157` → 4.85:1 |
| コメント | `#6b5065` | **2.79:1** ❌ | `#936F8B` → 4.60:1 |
| Doc コメント | `#7a6075` | **3.54:1** ❌ | `#997E94` → 5.44:1 |

**色相と彩度を保ったまま明度だけを解けば、意味体系を壊さずに比を上げられる。**
「読みやすさ」と「意味」はトレードオフではなかった ── *測っていなかっただけ*。

さらに、テーマの定義キーは **269 個あるうち 146 個しか書いていなかった**。
⚠️ **未定義のキーは消えず、base(vs-dark) の既定色に落ちる。**
だから「足りない」は画面上まったく見えない。
真値はエディタ本体のソース（`registerColor(` の全呼び出し）から取るしかない。

⇒ **原則 6: 意味を決めるのは設計、比とキー網羅を測るのは監査。設計だけでは閉じない。**
